...
`AssetSquirrel.CoreBusiness.Result<T>` (`Success` bool, `Message` string?, `Data` T?, plus `.Select<TOut>()` to remap `Data` while preserving `Success`/`Message`) is the return type for every mutating method (`Add/Update/Delete...Async`) across `PluginInterfaces`, plugin repository implementations, and UseCase interfaces/implementations — `Result<Entity>` at the repository layer, `Result<Dto>` at the UseCase layer (mapped via `.Select(e => e.Adapt<Dto>())`). On exception, the repository catches it, logs it via `IErrorsRepository.AddErrorAsync(service, class, method, exception)`, and returns `Result<T>.Fail(ex.Message)`. Read (`Get*Async`) methods are unaffected — they still return the list/DTO directly, not wrapped in `Result<T>`. In the Blazor UI, call sites unwrap `.Success` at the point of use (`EventCallback<bool>` signatures were kept as-is); `.Message` is not yet surfaced to the UI (backend-only for now).
